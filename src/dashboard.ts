...
byId('setup-form').addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget));if(data.password!==data.confirm_password){message('setup-message','Passwords do not match.',true);return}message('setup-message','Securing the password in this browser…');try{const saltBytes=crypto.getRandomValues(new Uint8Array(16));const verifier=await derivePasswordVerifier(data.password,saltBytes,passwordIterations);await api('/v1/dashboard/setup',{method:'POST',body:JSON.stringify({email:data.email,password_verifier:verifier,password_salt:toBase64Url(saltBytes),password_iterations:passwordIterations})});event.currentTarget.reset();await loadApp()}catch(error){message('setup-message',error.message,true)}});
byId('login-form').addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget));message('login-message','Verifying securely…');try{const parameters=await api('/v1/dashboard/login-parameters');const verifier=await derivePasswordVerifier(data.password,fromBase64Url(parameters.salt),parameters.iterations);await api('/v1/dashboard/login',{method:'POST',body:JSON.stringify({password_verifier:verifier})});event.currentTarget.reset();await loadApp()}catch(error){message('login-message',error.message,true)}});
