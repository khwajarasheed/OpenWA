...
const testEnv = (jobs: QueueJob[]): Env => ({
  ...
  PHONE_NUMBER_IDS: '',
  WABA_ID: '',
  INBOUND_QUEUE: { send: async (job: QueueJob) => { jobs.push(job); } },
} as unknown as Env);

describe('CORE webhook endpoint', () => {
  it('returns Meta’s challenge only when the verification token matches', async () => {
    const jobs: QueueJob[] = [];
    const accepted = await worker.fetch(new Request('https://core.example/webhooks/meta?hub.mode=subscribe&hub.verify_token=verify-me&hub.challenge=challenge-value'), testEnv(jobs));
