...
OpenWA is a self-deployable, developer-first WhatsApp CPaaS for a customer-owned Cloudflare account. It connects directly to the customer’s Meta Cloud API credentials; it does not broker messages or add a charge to Meta message costs.

This repository provides signed Meta webhook ingress, D1 contact/conversation/message storage, queue-backed outbound dispatch, per-phone rate coordination through Durable Objects, text/template send APIs, template synchronization, scoped service tokens, audit and webhook inspection, a self-hosted developer control plane, and a built-in MCP server.
...
The deployment screen has no ForgeScale Relay variables for the customer to fill. Keep the automatically selected D1 database, R2 bucket, jobs queue, and dead-letter queue, leave the detected deploy command as `npm run deploy`, leave **Protect with Cloudflare Access** off, then click deploy. Do not replace that command with the default `npx wrangler deploy`: the project command resolves the installation-specific D1 binding and applies migrations before publishing. Protecting the whole Worker would also block Meta's public webhook; path-specific Access can be added later. On first use, ForgeScale Relay generates an installation-only encryption key inside a customer-owned Durable Object. The key is not displayed, stored in Git, or sent to ForgeScale Relay infrastructure; it encrypts Meta credentials before they are stored in D1.

After deployment, click the deployed Worker URL and create the installation’s local owner account. The developer control plane is usable before WhatsApp is connected: it exposes realistic local fixtures, API and MCP setup, logs, webhook receipts, and prescribed sandbox delivery outcomes. Password verification and sessions remain inside the customer’s Worker and D1 database. Meta credentials are encrypted locally before D1 storage and are not sent to OpenWA-operated infrastructure.
...
**Connect WhatsApp** remains available from the control plane at all times. When the connection is validated, sends automatically transition from prescribed sandbox outcomes to the live Meta dispatch queue.
...
For a Git-connected Worker, set the Cloudflare **Deploy command** to `npm run deploy` rather than the default `npx wrangler deploy`. The deploy wrapper resolves an installation's D1 ID into an ignored local config, reuses or provisions the declared R2 and Queue resources, applies D1 migrations, and then publishes the Worker. This keeps the committed template portable for the Deploy Button while allowing a directly connected repository to deploy safely.
...
| `GET /v1/admin/tokens`, `DELETE /v1/admin/tokens/:id` | `admin` | List or revoke service tokens |
| `GET /v1/logs/audit`, `GET /v1/logs/jobs` | `logs:read` | Inspect local audit and dispatch history |
| `GET /v1/webhooks/receipts` | `webhooks:read` | Inspect locally retained webhook receipts |
...
| `GET`, `POST /v1/dashboard/tokens` | owner session | Inspect or create scoped service tokens |
| `GET`, `POST /v1/dashboard/team`, `DELETE /v1/dashboard/team/:id` | admin session | Manage Cloudflare Access-backed dashboard roles |
| `GET /v1/dashboard/logs`, `GET /v1/dashboard/webhooks` | owner session | Inspect developer observability data |
| `POST /v1/dashboard/sandbox/messages` | admin session | Run an egress-free prescribed delivery test before connecting WhatsApp |
| `POST /mcp` | scoped bearer token | Streamable HTTP MCP endpoint and tool calls |
