...
OpenWA CORE is designed to run in the customer’s Cloudflare account and call Meta directly. Customers remain responsible for Cloudflare account access, the local OpenWA owner password, and Meta permissions. Cloudflare Access may be added as an optional outer layer. OpenWA must not claim absolute data locality: data is processed by Cloudflare and Meta as part of normal operation.
